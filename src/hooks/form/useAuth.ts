...
export const useAuth = (data: string | null) => {
  ...
  const userLogin = async (data: any) => {
    dispatch(loginAction({ ...data })).then(({ payload }: any) => {
      try {
        loginForm.reset();
        cookies.set("accessToken", payload.data.tokens.accessToken, {
          path: "/",
        });
  ...
  const userRegister = async (data: any, role: string) => {
    dispatch(registerAction({ ...data })).then(({ payload }: any) => {
      try {
        registerForm.reset();
        cookies.set("accessToken", payload.data.tokens.accessToken, {
          path: "/",
        ...
        if (role.toUpperCase() === roles.ADMIN)
          return navigate("/auth/business-info?tab=1");
        navigate("/");
      } catch (error) {
        console.log("============API_ERROR===============");
        console.log(payload);
  ...
  const userBusinessAdd = async (data: any) => {
    dispatch(businessInformationAction({ ...data })).then(
      ({ payload }: any) => {
        try {
          navigate("/");
        } catch (error) {
  ...
  const userLogout = async (data: any) => {
    dispatch(logoutAction({ ...data })).then(({ payload }: any) => {
      try {
